		...
		for (int i = 1; i < animationParameterArr.length; i++) {
			...
			for (int j = 0; j < animationArr.length; j++) {
				visualizer.addFrameToAnimation(animationName,
						Integer.parseInt(animationArr[j].substring(1, animationArr[j].length() - 1).split("-")[0]),
						Integer.parseInt(animationArr[j].substring(1, animationArr[j].length() - 1).split("-")[1]));
			}
		}
	}
